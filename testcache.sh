...
NEOReadDebugKeys=1 AllowUnrestrictedSize=1 SGLANG_ENABLE_UNIFIED_RADIX_TREE=1 ZE_AFFINITY_MASK=6,7 NATS_SERVER="nats://127.0.0.1:5222" ETCD_ENDPOINTS="http://127.0.0.1:2479"  DYN_LOG="debug" python -m dynamo.sglang --device xpu --attention-backend intel_xpu   --model-path google/gemma-4-31B-it-qat-w4a16-ct   --page-size 64  --enable-hierarchical-cache --hicache-io-backend kernel_xpu  --hicache-size 100  --hicache-write-policy write_through --tp 2 --swa-full-tokens-ratio 0.2 --hicache-storage-backend nixl   --skip-tokenizer-init --log-level DEBUG --enable-request-time-stats-logging --enable-mfu-metrics --enable-metrics  > hicache3.log 2>&1
